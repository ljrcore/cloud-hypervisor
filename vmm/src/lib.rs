...
impl Vmm {
    ...
    fn connect_to_migration_socket(
        destination_url: &str,
        ...
        loop {
            ...
            match req.command() {
                ...
                Command::MemoryFd => {
                    ...
                    if let SocketStream::Unix(unix_socket) = &mut socket {
                        let mut buf = [0u8; 4];
                        ...
                        if existing_memory_files.is_none() {
                            existing_memory_files = Some(HashMap::new());
                        }
                        ...
                        Response::ok().write_to(&mut socket).map_err(|e| {
                            MigratableError::MigrateReceive(anyhow!(
                                "Error sending response: {}",
                                e
                            ))
...
impl RequestHandler for Vmm {
    ...
    fn vm_receive_migration(
        ...
        let socket = Vmm::accept_migration_socket(&receive_data_migration.receiver_url)?;

        self.receive_migration(socket)
    }
    ...
    fn vm_send_migration(
        ...
        let socket = Vmm::connect_to_migration_socket(&send_data_migration.destination_url)?;
